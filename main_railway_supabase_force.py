...
async def initialize_supabase_force():
    """Force Supabase connection - NO FALLBACKS"""
    global db_handler, db_type
    
    print("🚀 Railway Force Supabase Mode")
    print("🔒 FORCING SUPABASE - NO SQLITE AT ALL!")
    ...
    try:
        ...
        if test_result is not None:
            ...
            # CRITICAL: Override the global database handler in all modules
            import METHODS.operations as ops
            ops.db_handler = rest_client
            ops.db_type = "supabase_rest"
            
            import METHODS.labs_handler as labs
            labs.db_handler = rest_client
            labs.db_type = "supabase_rest"
            
            import METHODS.lab_operations as lab_ops
            lab_ops.db_handler = rest_client
            lab_ops.db_type = "supabase_rest"
            
            import METHODS.manager_operations as mgr_ops
            mgr_ops.db_handler = rest_client
            mgr_ops.db_type = "supabase_rest"
            
            print("✅ Database handler overridden in all modules!")
            return True
        else:
    ...
    try:
        ...
        # CRITICAL: Override the global database handler in all modules
        import METHODS.operations as ops
        ops.db_handler = supabase_db
        ops.db_type = "supabase_postgres"
        
        import METHODS.labs_handler as labs
        labs.db_handler = supabase_db
        labs.db_type = "supabase_postgres"
        
        import METHODS.lab_operations as lab_ops
        lab_ops.db_handler = supabase_db
        lab_ops.db_type = "supabase_postgres"
        
        import METHODS.manager_operations as mgr_ops
        mgr_ops.db_handler = supabase_db
        mgr_ops.db_type = "supabase_postgres"
...
async def main(bot):
    try:
        ...
        success = await initialize_supabase_force()
...
if __name__ == "__main__":
    ...
    print("🤖 Starting KITS Bot (Railway Force Supabase Version)...")
