...
async def initialize_supabase_only():
    """Initialize ONLY Supabase - NO SQLite fallback"""
    global db_handler, db_type
    
    print("🚀 Railway Deployment - Supabase ONLY Mode")
    print("🔒 SQLite fallback DISABLED - Supabase required!")
    
    # Check if we're in container deployment
    container_deployment = os.environ.get("CONTAINER_DEPLOYMENT", "false").lower() == "true"
    force_rest = os.environ.get("FORCE_SUPABASE_REST", "false").lower() == "true"
    
    print(f"📦 Container Deployment: {container_deployment}")
    print(f"🌐 Force REST API: {force_rest}")
    
    # Method 1: Try Supabase REST API first (most reliable on Railway)
    if force_rest or True:  # Always try REST first on Railway
        try:
    ...
    # NO FALLBACK - FAIL IF SUPABASE DOESN'T WORK
...
async def main(bot):
    try:
        ...
        success = await initialize_supabase_only()
...
if __name__ == "__main__":
    ...
    print("🤖 Starting KITS Bot (Railway Supabase-Only Version)...")
