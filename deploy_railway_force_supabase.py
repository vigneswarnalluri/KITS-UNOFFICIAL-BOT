#!/usr/bin/env python3
"""
Deploy Railway with FORCE Supabase mode
This overrides database handlers in all modules to force Supabase usage
"""

import subprocess
import sys
import os

def main():
    print("🚀 Deploying Railway FORCE Supabase Mode")
    print("=" * 50)
    
    # Check if we're in the right directory
    if not os.path.exists("main_railway_supabase_force.py"):
        print("❌ main_railway_supabase_force.py not found!")
        return False
    
    if not os.path.exists("requirements.txt"):
        print("❌ requirements.txt not found!")
        return False
    
    print("✅ Files ready for force deployment")
    print("📋 Force Supabase mode includes:")
    print("  ✅ Overrides database handlers in ALL modules")
    print("  ✅ Forces Supabase usage even if modules try SQLite")
    print("  ✅ All dependencies included")
    print("  ✅ Environment variable validation")
    
    try:
        print("\n📝 Adding files to git...")
        subprocess.run(["git", "add", "."], check=True)
        
        print("📝 Creating commit...")
        commit_message = "Railway: FORCE Supabase mode - override all database handlers"
        subprocess.run(["git", "commit", "-m", commit_message], check=True)
        
        print("🚀 Pushing to Railway...")
        subprocess.run(["git", "push", "origin", "main"], check=True)
        
        print("✅ FORCE Supabase mode deployed!")
        print("⏳ Railway will build with forced Supabase usage")
        print("📱 Check Railway dashboard for build status")
        
        return True
        
    except subprocess.CalledProcessError as e:
        print(f"❌ Git operation failed: {e}")
        return False
    except Exception as e:
        print(f"❌ Unexpected error: {e}")
        return False

def show_force_mode_explanation():
    print("\n" + "=" * 60)
    print("🔧 FORCE SUPABASE MODE EXPLANATION")
    print("=" * 60)
    print("This version FORCES Supabase usage by:")
    print("✅ Overriding db_handler in ALL modules (operations, labs, etc.)")
    print("✅ Preventing any module from using SQLite directly")
    print("✅ Ensuring all database operations go through Supabase")
    print("✅ Crashing if Supabase fails (no fallback)")
    print()
    print("🎯 This should FINALLY eliminate the 'no such table: sessions' error!")

def show_railway_env_instructions():
    print("\n" + "=" * 60)
    print("🔧 CRITICAL: RAILWAY ENVIRONMENT VARIABLES")
    print("=" * 60)
    print("Copy these EXACTLY to your Railway project environment variables:")
    print()
    
    env_vars = [
        "CONTAINER_DEPLOYMENT=true",
        "API_ID=27523374",
        "API_HASH=b7a72638255400c7107abd58b1f79711",
        "BOT_TOKEN=8007204996:AAGbfj4e6sEefgdI8Ixncl3tVoI6kKnZo28",
        "SUPABASE_USER=postgres",
        "SUPABASE_PASSWORD=Viggu@2006",
        "SUPABASE_DATABASE=postgres",
        "SUPABASE_HOST=db.wecaohxjejimxhbcgmjp.supabase.co",
        "SUPABASE_PORT=5432",
        "SUPABASE_URL=https://wecaohxjejimxhbcgmjp.supabase.co",
        "SUPABASE_ANON_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndlY2FvaHhqZWppbXhoYmNnbWpwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTkyMjk1NzQsImV4cCI6MjA3NDgwNTU3NH0.MPOSqIjbPLd1zoqwjsCZQBQSeUBMQdRND7lnMOmbCfk",
        "FORCE_SUPABASE_REST=true",
        "DISABLE_SQLITE_FALLBACK=true",
        "SUPABASE_PRIORITY=rest",
        "RAILWAY_SUPABASE_ONLY=true"
    ]
    
    for var in env_vars:
        print(f"  {var}")
    
    print()
    print("⚠️  CRITICAL: Do NOT set these variables (they cause localhost fallback):")
    print("  ❌ POSTGRES_HOST=localhost")
    print("  ❌ POSTGRES_USER_ID=postgres") 
    print("  ❌ POSTGRES_PASSWORD=viggu")
    print("  ❌ POSTGRES_DATABASE=kits_bot_db")

def show_expected_results():
    print("\n" + "=" * 60)
    print("🎯 EXPECTED RESULTS")
    print("=" * 60)
    print("✅ Build succeeds (all dependencies included)")
    print("✅ Bot starts with Supabase connection")
    print("✅ NO 'no such table: sessions' errors")
    print("✅ All modules forced to use Supabase")
    print("✅ All features work (login, attendance, marks, etc.)")
    print("❌ If Supabase fails, bot crashes (better than SQLite errors)")

if __name__ == "__main__":
    print("🤖 Railway FORCE Supabase Deployment")
    print("This overrides ALL database handlers to force Supabase")
    print()
    
    show_force_mode_explanation()
    show_railway_env_instructions()
    show_expected_results()
    
    print("\n" + "=" * 60)
    print("🚀 READY TO DEPLOY FORCE MODE?")
    print("=" * 60)
    print("⚠️  WARNING: This version will CRASH if Supabase fails")
    print("✅ But it will FORCE Supabase usage in ALL modules")
    print("🎯 This should FINALLY fix the SQLite fallback issue!")
    
    response = input("Deploy FORCE Supabase mode? (y/N): ").strip().lower()
    
    if response in ['y', 'yes']:
        success = main()
        if success:
            print("\n🎉 FORCE Supabase mode deployed!")
            print("⏳ Wait 2-3 minutes for Railway to build")
            print("📱 Check Railway dashboard for status")
            print("\n🔧 IMPORTANT: Make sure environment variables are set correctly!")
            print("   This should FINALLY eliminate the SQLite fallback!")
        else:
            print("\n❌ Deployment failed. Check errors above.")
    else:
        print("\n⏸️ Deployment cancelled.")
